...
function validateBranchesDepth(branches, depth, path, errors) {
  if (depth > 30) {
    errors.push({
      instancePath: path,
      message: `Branch depth exceeds 30 at path: ${path}`,
    })
    return
  }
  ...
  branches.forEach((branch, index) => {
    ...
    if (branch.product) {
      // Validate the product resides at an acceptable depth.
      if (depth > 30) {
        errors.push({
          instancePath: `${currentPath}/product`,
          message: `Product depth exceeds 30 at path: ${currentPath}/product`,
        })
      }
    }
  })
